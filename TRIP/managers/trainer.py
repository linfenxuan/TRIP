...
class Trainer():
    ...
    def train_epoch(self):
        ...
        for b_idx, batch in enumerate(dataloader):
            ...
            loss = self.criterion(score_pos, score_neg.view(len(score_pos), -1).mean(dim=1), torch.Tensor([1]).to(device=self.params.device)).cpu()
            print(f"loss: {loss}")
            ...
            if self.params.coef_dgi_loss:
                ...
                dgi_loss = self.b_xent(logits, lbl).cpu()
